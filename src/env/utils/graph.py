...
class Grapher():
    ...
    def graph_params(self, rewards, dist, actor_loss, critic_loss, time, reward_vel, reward_time, reward_height, reward_rotation, steps):
        ...
        x_s = np.linspace(1, steps, num=steps)

        ax[0,0].plot(x_s, reward_vel)
        ax[0,0].set_title('reward_vel')
        ax[0,1].plot(x_s, reward_time)
        ax[0,1].set_title('reward_time')
        ax[1,0].plot(x_s, reward_rotation)
        ax[1,0].set_title('reward_rotation')
        ax[1,1].plot(x_s, reward_height)
