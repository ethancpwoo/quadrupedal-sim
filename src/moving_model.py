...
#p.changeDynamics(robot, 8, jointLowerLimit=-60, jointUpperLimit=60)

#set the center of mass frame (loadURDF sets base link frame) startPos/Ornp.resetBasePositionAndOrientation(boxId, startPos, startOrientation)
for i in range (1000):
    p.setJointMotorControl2(robot, 9, controlMode=mode, targetVelocity=-10)
    p.setJointMotorControl2(robot, 10, controlMode=mode, targetVelocity=-10)
    p.stepSimulation()
    time.sleep(1./240.)
